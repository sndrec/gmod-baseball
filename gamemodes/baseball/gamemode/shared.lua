...
hook.Add("UpdateAnimation", "DiveAnim", function(pl, vel, maxGroundSpeed)
...
hook.Add("PlayerTick", "DiveTick", function(pl, mv)

	--print(pl:GetSequence())
	--pl:ResetSequenceInfo()
	--pl:ResetSequence( 133 )
end)

local function Accelerate(pl, move, wishdir, wishspeed, accel)
	local playerVelocity = move:GetVelocity()
	local oldVel = move:GetVelocity():Length()
...
hook.Add("PlayerTick", "Strafejump", function(pl, move)
	...
	if pl:OnGround() then
		local MaxSpeed = 320 + (pl:GetLegsStat() * 8)
		local StaminaDrainMult = math.Remap(pl:GetBodyStat(), 0, 8, 1.1, 0.5)
		if move:KeyDown(IN_SPEED) and pl:GetStamina() >= 0 then
			pl:SetStamina(pl:GetStamina() - FrameTime() * 5 * StaminaDrainMult)
			local Remap = math.Clamp(math.Remap(pl:GetStamina(), 0, pl:GetMaxStamina() * 0.25, 1, 2.5), 1, 2.5)
			MaxSpeed = MaxSpeed * Remap
			pl:SetMaxStamina(pl:GetMaxStamina() - FrameTime() * 0.00012 * pl:GetMaxStamina())
		else
			pl:SetMaxStamina(math.min(100, pl:GetMaxStamina() + FrameTime() * 0.001))
			pl:SetStamina(math.min(pl:GetStamina() + FrameTime() * 2, pl:GetMaxStamina()))
			--print(pl:GetStamina())
		end
		...
		local accelRate1 = (move:GetVelocity():GetNormal():Dot(forward) * 0.5) + 0.5
		local accelRate2 = 1 - ((move:GetVelocity():GetNormal():Dot(wishdir) * 0.5) + 0.5)
		--if wishspeed == 0 then
		--	accelRate = 0.125
		--end
		--print(accelRate1)
		--print(pl:GetRunVector())
		local decelFactor = 0

		local accelAdd = pl:GetLegsStat() * 0.0125

		local newRunVector = LerpVector(FrameTime() * (math.max(accelRate1, accelRate2)) * (0.2 + accelAdd), pl:GetRunVector(), wishdir)
		if newRunVector:LengthSqr() < pl:GetRunVector():LengthSqr() then
			newRunVector = LerpVector(FrameTime() * (math.max(accelRate1, accelRate2)) * 4, pl:GetRunVector(), wishdir)
		end
		pl:SetRunVector(newRunVector)
		local dot = (wishdir:Dot(pl:GetRunVector()) * 0.5) + 0.5
		--print(dot)
		local strafeAccel = math.max((dot * 1.25) + 0.5, 0)
		--print(pl:GetRunVector())
		Accelerate(pl, move, wishdir, wishspeed, strafeAccel)
	end
end)

batterCamPos = Vector(-5050, -5050, 150)
batterCamAng = Angle(4, 45, 0)

hook.Add( "StartCommand", "BaseballMove", function( pl, cmd )
	if pl:Alive() then
		if pl:GetActiveWeapon():GetClass() == "baseball_bat" then
			if pl:GetPos():Distance(Vector(-4754.198730, -4755.620605, 64.031250)) < 500 then
				...
				--print(wishmove)
				cmd:ClearMovement()
				cmd:SetSideMove(wishmove.x)
				cmd:SetForwardMove(wishmove.y)
